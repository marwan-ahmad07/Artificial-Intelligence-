...
def solve_mac_trace(board):
    ...
    if not ac3(domains):
        # If initial propagation already fails, the puzzle is inconsistent.
        tree_lines.append("Root Node: Hard Board (Initial AC-3 failed)")
        return tree_lines
        
    def _backtrack_mac(domains, depth=0):
        # Keep the trace readable by stopping after a few levels.
        if depth > 4: 
            tree_lines.append("  " * depth + "|-- ... (Search continues successfully to solution)")
            return True
            
        # Only cells with more than one candidate still need to be decided.
        unassigned = [v for v in domains if len(domains[v]) > 1]
        if not unassigned:
            return True
            
        # Use MRV again so the trace matches the actual search strategy.
        var = min(unassigned, key=lambda v: len(domains[v]))
        
        for val in domains[var]:
            # Each line records one guess in the search tree.
            tree_lines.append("  " * depth + f"|-- Guess: Cell {var} = {val}")
            
            # Branch state must be isolated from other guesses.
            new_domains = copy.deepcopy(domains)
            new_domains[var] = [val]
            
            # Re-propagate constraints from this guess before going deeper.
            queue = []
            ...
            while queue:
                ...
                if revise(new_domains, xi, xj):
                    if len(new_domains[xi]) == 0:
                        # A wiped-out domain means this branch cannot succeed.
            ...
            if is_consistent:
                # This branch is still valid after propagation.
